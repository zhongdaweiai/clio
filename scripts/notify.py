...
def main() -> int:
    if len(sys.argv) < 2 or sys.argv[1] not in ("scan", "resolutions"):
        log.error("usage: notify.py scan | resolutions")
        return 2
    ...
    if sys.argv[1] == "scan":
        ...
        if not latest:
            ...
            return 0
        path, scan = latest
        # Only email if today's date matches the file (avoid emailing stale scans).
